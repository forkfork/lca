"""External behavioral probes; never copied into the agent's workspace."""
import json
import os
...
sys.path.insert(0, os.environ['ORDER_WORKSPACE'])
...
class CancellationContract(unittest.TestCase):
    ...
    def cli(self,*args):
        return subprocess.run([sys.executable,'-B','-m','orders','--database',self.path,*args],
                              cwd=os.environ['ORDER_WORKSPACE'],capture_output=True,text=True,timeout=10)
