"""Fixed-tests workload grading; all supplied tests are immutable."""
...
def grade(workspace, trajectory):
    ...
    mutations=[i for i,e in enumerate(events) if e.get('name') in {'apply_patch','apply_patch','edit','multi_edit','write','file_change','mutation'} and not e['result'].get('is_error')]
    ...
    gates={'public_regressions_and_examples':public_ok,'hidden_cancellation_contract':hidden_ok,
           'scope_preserved':bool(scope),'supplied_tests_unchanged':not any(n.startswith('tests/') for n in changed+added),'version_one_migration_preserved':migration_preserved,
    ...
    return {'passed':all(gates.values()),'score':100*sum(gates.values())/len(gates),'hard_gates':gates,
            'evidence':{'fixture_version':'2-fixed-tests-1','artifact_correct':public_ok and hidden_ok,'changed_files':changed,'added_files':added,
                        'changed_lines':changed_lines,'mutation_calls':len(mutations),
                        'patch_calls':sum(e.get('name')=='apply_patch' for e in events),
                        ...
                        'failed_mutations':sum(e.get('name') in {'apply_patch','apply_patch','edit','multi_edit','write'} and bool(e['result'].get('is_error')) for e in events),
