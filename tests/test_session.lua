...
run_test("Bedrock sessions default to Astra and preserve explicit model choices", function()
	...
	for _, model in ipairs({ "", "gpt-6-astra", "global.openai.gpt-5.6-sol" }) do
		write_file(path, '{"provider":"bedrock","providers":{"bedrock":{"apiKey":"test","model":"' .. model .. '"}}}')
		require("agent.providers")._invalidate_cache()
		assert_eq(require("agent.providers").default_model(path), "gpt-6-astra")
		assert_eq(session_module.create({ credentials_path = path }).model, "gpt-6-astra")
		for _, explicit in ipairs({ "gpt-6-astra", "gpt-5.6-sol" }) do
			assert_eq(session_module.create({ credentials_path = path, model = explicit }).model, explicit)
		end
	end
	os.remove(path)
	require("agent.providers")._invalidate_cache()
