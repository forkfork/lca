...
- For small greenfield technical choices, prefer boring local defaults: env vars for config, SQLite or plain files for local state depending on complexity, structured JSON for machine-readable output, newline-delimited logs for append-only event traces, and simple Makefile/shell commands only when they remove repeated command friction.
- For HTTP APIs, default to JSON request/response bodies, health checks, bounded request sizes, explicit status codes, simple error objects, and curl-based smoke tests. Avoid GraphQL, OpenAPI generation, service meshes, queues, or async workers unless requested or clearly needed.
