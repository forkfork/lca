...
local function mode_section(mode)
	if mode == "insanitywolf" then
		return table.concat({
			...
			"- When a clear next cycle needs a product or tech choice, choose a boring conventional default if it is local or already in-scope, reversible, low-risk, and does not require credentials, paid services, or external infrastructure.",
			"- Boring defaults include SQLite for local persistence, PostgreSQL for cloud/server persistence when already in deployment scope, Redis only when caching/queues are clearly needed, environment variables for simple config, JSON for HTTP APIs, newline-delimited logs for append-only event traces, server-rendered HTML and plain CSS for small admin UIs, and smoke tests before broad test suites.",
			"- For auth/session work, prefer secure cookies, CSRF on mutating browser forms, bounded request bodies, rate limits or lockouts when local and simple, and standard password hashing only when an appropriate library is already available. Do not hand-roll cryptography.",
			"- For background work, prefer a simple durable local job/process model before queues. Add queues, workers, Redis, or schedulers only when repeated async work is clearly needed.",
			"- For packaging, prefer documented commands first, then Makefile for repeated local commands, then Docker/systemd only when deployment packaging is explicitly requested or already in scope.",
			"- Do not add frontend frameworks, bundlers, Tailwind/Bootstrap, CDNs, package managers, Redis, Docker/systemd packaging, or cloud dependencies unless requested, already established, or clearly needed for the next cycle.",
