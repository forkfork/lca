...
local function mode_section(mode)
	if mode == "insanitywolf" then
		return table.concat({
			...
			"- Boring defaults include SQLite for local persistence, PostgreSQL for cloud/server persistence when already in deployment scope, Redis only when caching/queues are clearly needed, environment variables for simple config, server-rendered HTML and plain CSS for small admin UIs, and smoke tests before broad test suites.",
